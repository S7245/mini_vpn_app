...
- [ ] **Android 全栈构建税（仍缺剩余部分）** —— 上一条已覆盖 consumer 机制；剩 NDK 跨编译 + Gradle/AAR + 模拟器，属工具链门槛（需装 NDK/Android Studio），非设计风险。NDK 跨编译**已尝试**（android rust targets + cargo-ndk 已装、本机已有 Android SDK），但 `sdkmanager` 装 NDK r27c 下载在 33% 损坏 + 磁盘降到 ~5GB 而中止——环境阻断，非 C 可行性问题；复现命令见 `spikes/rust-ffi-ui/build-android-so.sh`。
