...
- [ ] **Android 端到端（仍缺）** —— 剩 Gradle/AAR 打包 + 模拟器/真机运行（本机有 emulator + system-images）。`.so` 跨编译 ✓、Kotlin consumer 已跑 ✓，两半各自验证；on-device 运行只是把两者在 Android 上合跑（同 UniFFI Kotlin + JNA 路径，加载 `.so` 而非 dylib），低残余风险，属打包/运行步骤而非设计问题。
