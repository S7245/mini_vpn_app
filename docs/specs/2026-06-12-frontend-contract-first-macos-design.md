...
- [ ] **Android 全栈构建税（仍缺）** —— Kotlin 绑定已能生成（`uniffi-bindgen --language kotlin` 出 2057 行 `.kt`，`ControlService`/`EventObserver`/`ControlEvent` + async 映射为 `suspend fun`，**同一 Rust core 免费得到 idiomatic Kotlin API**）。但本机**无** android rust targets / NDK / Gradle，**未测**：跨编译 `.so`（每 ABI）、Gradle 模块依赖 JNA、coroutine/main-dispatcher hop（类比 Swift 的 MainActor hop）。这是选 C 后的下一个 spike，需机器装 Android Studio/NDK。
