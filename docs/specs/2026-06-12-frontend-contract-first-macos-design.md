...
> **修订（2026-06-19）**：首平台"原生 macOS 先行"依然成立（macOS 客户端已按此建成）。
> 当初**延后的"全局多端栈决策"现已收口 = C**（共享 Rust core 经 FFI + 每平台薄原生 UI），
> 依据 §7.1 复盘的 5 段 spike 实测。见 [ADR 2026-06-19](../adr/2026-06-19-gui-stack-c-shared-rust-core.md)。
...
> **复盘已收口（2026-06-19）：gate CLOSED。决定 = 选 C（共享 Rust core 经 FFI + 每平台
> 薄原生 UI）。** B 出局、A 为已知次优 fallback。决定性依据：VPN 数据面已 Rust → 网络扩展
> 每平台无论 A/B/C 都得 FFI-host 它，C 沿同一道边界共享控制逻辑近乎零边际、纯收益。
> 完整决策记录见 [ADR 2026-06-19](../adr/2026-06-19-gui-stack-c-shared-rust-core.md)。下方
> 触发器/输入/判据为复盘留痕。
...
- [x] 出现要落地一个**非 Apple 平台（Android / Windows）**的具体决定（有真实用户/商业
  理由）。注意：iPhone / iPad 复用同一套 SwiftUI，**不触发**复盘——那只是增加 target。
  → **已触发（2026-06-16）：第二平台定为 Android。** 复盘正式启动，下方输入清单即其
  证据收集；这些 FFI spike 是为它服务的，尚未做最终 A/B/C 决定（仍缺若干输入，见下）。
...
- [x] **VPN 网络扩展的不可避税（Phase 5，2026-06-19）—— 已测，结论决定性** —— 做了 compile-only
  的 `NEPacketTunnelProvider` 脚手架（`spikes/macos-netext/`，`** BUILD SUCCEEDED **`；因缺
  NetworkExtension entitlement/provisioning 不能加载——那 entitlement 本身就是税）。**决定性发现**：
  mini_vpn 的数据面本体已是 Rust，所以**每平台的网络扩展都必须在进程内 FFI-host 那个 Rust 数据面核，
  无论选 A/B/C**——FFI 边界不是 C 的额外成本，是 VPN 形态的既定前提。这把 A vs C 压扁：两者都付薄的
  NE 原生胶水 + 都 FFI 数据面核，唯一差别是控制/业务逻辑 A 每平台重写、C 沿同一道 FFI 边界写一次
  （边际≈0、纯收益）。完整分析见 `spikes/macos-netext/TAX.md`。§2.1"Flutter 只省 GUI 一层"在 VPN 上
  不仅成立，且对 A 同样适用（A 的"全原生逻辑"是半假象，隧道仍 Rust-over-FFI）。
- [ ] **AI 写原生 UI 的实际速度**：SwiftUI 这程的出活速度能否复制到 Kotlin / C# / Dart。
- [ ] **契约稳定度**：①/② 是否已稳到"第二客户端只是重新绑定契约"。
- [x] **FFI 事件流→原生 UI 的摩擦** —— 已做去风险 spike（`docs/spikes/2026-06-16-rust-ffi-ui-spike.md`，friction log 见 `spikes/rust-ffi-ui/FINDINGS.md`，2026-06-16）。结论：Rust core 经 UniFFI 0.31 把 **live 事件流（state/stats/log）推给原生 Swift 是轻的**——~4 文件 / 3 命令 / ~210 LOC，proc-macro 无 UDL，静态链接零系统库折腾，行为与 Swift `MockControlService` oracle 完全一致。唯一强制成本：**回调在 Rust 发射线程到达、无自动主线程跳**，绑 `@MainActor @Published` 需一次强制 hop（`Task { @MainActor in … }`，样板、低风险）。构建唯一坑：要 *编译* 生成的 `.swift` 且经 `-Xcc -fmodule-map-file` 喂 FFI modulemap。**押 C 前仍需去风险的两项**：(a) Xcode/xcframework 构建集成 vs 裸 `swiftc`；(b) tokio/async 跨边界的 foreign-executor 层（spike 用裸 OS 线程绕开了）。
- [x] **(a) xcframework + SwiftUI 构建税（Phase 2，2026-06-16）** —— 已测：release 静态库打 `xcframework` → xcodegen 链接 → `xcodebuild` **BUILD SUCCEEDED** → 真 SwiftUI app 跑通，Rust ticker 经 FFI → MainActor → `@Published` 全程 `isMain=true`（10/10）。构建税**低**：比 CLI 多 3 步（modulemap 重命名为 `module.modulemap`、`-create-xcframework`、xcodegen 一行依赖）。MainActor 绑定确认低风险。
- [x] **(b) tokio/async 跨边界（Phase 3，2026-06-16）** —— 已测：加 `tokio` 依赖 + `uniffi` 的 `tokio` feature + `#[uniffi::export(async_runtime="tokio")]`（共约 3 行配置），Swift 以原生 `await` 消费（`await ping()`、`await streamTicks()` 经回调推流均成功）。**tokio-based core 经 FFI 低摩擦。**
- [~] **Android consumer 机制（Phase 4，2026-06-17）—— 已在 host JVM 跑通** —— 不止生成绑定：UniFFI 生成的 Kotlin 在 OpenJDK 17 上经 JNA 加载 Rust dylib 实跑，**回调接口 / `sealed class ControlEvent` / `enum ConnectionState` / tokio `suspend fun`（`await ping()`、`streamTicks`）全部工作**（Kotlin 代码路径与 Android 一致，仅 native target 不同：`.so` vs `.dylib`）。线程同 Swift：std::thread 回调在 JNA worker 线程、tokio async resume 在调用 coroutine——Android 需同样的 `Dispatchers.Main` hop（类比 MainActor）。**仍缺**：NDK 跨编译 `.so`（每 ABI；android rust targets 已装、NDK 未装）、Gradle/AAR 打包、模拟器/真机运行。摩擦点：`brew install kotlin` 因 openjdk 依赖下载失败，改用独立 kotlin-compiler zip + 系统 JDK 绕过。
- [x] **Android `.so` 跨编译（Phase 4b，2026-06-18）—— 已完成** —— tokio/async 的 Rust core 经 `cargo ndk` 跨编译为真 Android 库：`android-libs/arm64-v8a/libminivpn_ffi.so` = `ELF 64-bit ARM aarch64 shared object`（API 24，~90s）。坑：`sdkmanager` 装 NDK r27c 连续两次在 33% 截断损坏（与磁盘无关，129GB 仍失败）→ 改从 dl.google.com 直接 `curl -C --retry` 下 r25c .zip 绕过；`cargo-ndk 4.x` 的 API level 是 `--platform N` 不是 `-p N`（`-p` 是 cargo 的 `--package`）。脚本见 `spikes/rust-ffi-ui/build-android-so.sh`。
- [x] **Android 端到端（Phase 4c，2026-06-19）—— 已在真模拟器跑通** —— 最小 Gradle app 打进 arm64 `.so` + 生成 Kotlin + JNA(aar) + coroutines，装上 arm64 AVD（`Medium_Phone_API_36.0`，headless 启动）运行，`adb logcat` 抓到完整 Rust→`.so`→JNA→Kotlin 事件流：connect/stats/log、ticker（后台线程）、**`await ping()` → "pong (awaited on tokio runtime)"**、streamTicks、done——与 Swift app、host-JVM 行为完全一致。坑：AGP 默认自动装 `build-tools;34.0.0` 又撞下载损坏 → pin `buildToolsVersion="35.0.1"`（已装版本）绕过；JNA 在 Android 须用 `@aar`（捆每 ABI 的 JNA dispatch `.so`）。脚本见 `spikes/rust-ffi-ui/build-android-app.sh`。**Android FFI 设计风险全部退清。**
- [ ] **Android VpnService 的网络扩展不可避税**（与上面"VPN 网络扩展"项呼应）：Android 端有多少原生工作是无论 GUI 栈都得做的——这决定 C 相对 A 的真实省量，**未测**。
...
- [x] **C（新）—— 经 Phase 1–5 spike，现为首选**：把可移植层（service/VM/models）沉到
  **共享 Rust core 经 FFI**，每平台原生薄壳 UI。macOS 这程已证明 UI 极薄、逻辑都在 Core；
  数据面 core 本就是 Rust、契约又语言中立。FFI 摩擦已实测退清（Apple xcframework+SwiftUI、
  Android `.so`+Kotlin 均 on-device 实跑；sync + tokio/async 都通）。**决定性**：VPN 数据面
  已 Rust → 网络扩展每平台都得 FFI-host 它（A/C 都付），故 C 把控制逻辑也放进同一道边界是
  零边际、纯收益（见 `spikes/macos-netext/TAX.md`）。
  - A：仍可行、运营最简（无 FFI 打包、原生调试栈），但控制/业务逻辑每平台重写、单人维护漂移税复利。
  - B：VPN 场景出局（网络扩展每平台仍原生，Flutter 只省简单 GUI 还加一道桥）。
