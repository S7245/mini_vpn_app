...
### Phase 4 — 切片② BackendService + 按模块实现 ✅ 完成（commits `48e593d`→`350fe54`，2026-06-26）
- **rust-core 切片②**（commit `48e593d`）：`models.rs`（TokenPair/Subscription/Device/DeviceList/SelectBestResponse=uniffi::Record + `Node`=uniffi::Enum，serde 内部 tag by `kind`；Rust snake_case 字段直吃契约 JSON，UniFFI 出 camelCase Kotlin）；`backend.rs` async `BackendService`（UniFFI tokio → Kotlin suspend funs）mock 解码 `include_str!` 嵌入的 `contracts/mock` fixtures（`scripts/sync-mocks.sh` 同步）；`BackendError`。10 cargo 测试绿（4 control + 6 backend）。镜像 Swift `MockBackendService`/models。
- 按模块（Compose 屏 + 薄 Kotlin VM over Rust service，VM 依赖生成的 `BackendServiceInterface` → host JVM 可测）：
  - **A1 Auth**：`PrefsSessionStore`（SharedPreferences，同步、mock 明文；真实现换 EncryptedSharedPreferences 同接口，Q-A4）+ `AuthViewModel`（init 恢复会话 Q-03 / login·register 持久化 / logout 清除 / 错误映射）+ Login/Register 接 VM。
  - **A2 Connect**：FR-09——`connect(nodeId)` 接手选节点、否则 `auto()`；节点卡显示共享 `NodeListViewModel` 的选中节点；实时流量沿用 Phase 2 FFI 链路。
  - **A3 Nodes**：`NodeListViewModel`（load / 手选 / selectBest），实例与 Connect 共享（FR-09）；过期 dedicated 经 `java.time` 置灰（启用 core library desugaring）。
  - **A4 Account**：`AccountViewModel`（订阅 / 设备 / canRevoke Q-02 / revoke）；ISO 日期经 java.time 格式化。
- DI：`MiniVpnApplication` 持 app 级单例 `AppContainer`（backend + SessionStore，跨 config change 存活）+ ViewModel factory 经 CompositionLocal 注入。
- 测试：12 个 Kotlin VM 单测（coroutines-test）+ 10 cargo 测试，全绿。
- 验收（AVD arm64 逐屏截图）：login→gate；Nodes 显示 LA/Tokyo/San Jose（契约 fixture over FFI）；FR-09 手选节点驱动 Connect（节点卡「manually selected」+ 实时 stats）；Account monthly/active/Jul 12 2026 + Sam's MacBook；logout 回登录；旋转不崩、保持登录。
- `/code-review` 已跑（rust-core 无 finding；Android 修 2 处：BackendService 提升为 app 单例避免旋转 churn native handle、VM catch 重抛 CancellationException）。
- 注：左滑解绑仍受 `adb input swipe` 无法驱动 Compose AnchoredDraggable 限制，真机手势 instrumented 测试待补（VM 的 revoke/canRevoke 逻辑已单测覆盖）。
...
- **Q-A4 SessionStore 实现**：~~DataStore vs EncryptedSharedPreferences~~ → **已定：plain SharedPreferences（同步、mock 明文）**；真实现换 EncryptedSharedPreferences（同步同接口，比 DataStore 异步 init 无闪屏）。Phase 4。
