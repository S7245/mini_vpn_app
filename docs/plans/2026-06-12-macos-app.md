...
**Known environment dependencies (not gaps):** `swift test` needs the Swift toolchain (always present on macOS dev). The Task 8 app-shell build needs Xcode + XcodeGen; if absent, it is skipped and noted — the Core package (all logic + tests) is fully validated without it. The image snapshot test is host-rendering-dependent and is therefore OFF by default (gated behind `SNAPSHOT=1`); the default `swift test` suite is deterministic and headless-safe, and the traffic formatters are covered by pure-function tests (`TrafficFormattingTests`). The ① `ControlService` boundary is hardened up front — `send` is `async throws`, the event stream carries `.error`, and `MockControlService` is an `actor` (no data race under the live ticker) — so swapping the mock for a real transport is a one-line change with no call-site/UI impact.
