...
**Constraints:**
- All artifacts land in `mini_vpn_app-macos/macos-app/` (the `feat/macos-app` worktree). The `mini_vpn` core repo is NEVER touched. Commit to `feat/macos-app`; push to `origin/feat/macos-app` — NEVER push `main`.
...
Run:
```bash
chmod +x /Users/liushan/Documents/Personal/Languages/Rust/mini_vpn_app-macos/macos-app/scripts/sync-mocks.sh
/Users/liushan/Documents/Personal/Languages/Rust/mini_vpn_app-macos/macos-app/scripts/sync-mocks.sh
```
Expected: `synced mocks -> .../Resources/Mocks`; 7 JSON files copied (user, token-pair, subscription, device, device-list, node-list, select-best). The `contracts/mock/local-control/*.json` ① fixtures are NOT synced here — the ① mock is a deterministic hand-coded state machine (Task 4), not a fixture reader.
...
Run:
```bash
cd /Users/liushan/Documents/Personal/Languages/Rust/mini_vpn_app-macos/macos-app/Core
...
```bash
cd /Users/liushan/Documents/Personal/Languages/Rust/mini_vpn_app-macos
...
Note: the `node-list.json` contract mock contains THREE nodes — two `shared` (LA standard, Tokyo premium) and one `dedicated` (San Jose static IP). `contracts/` is authoritative and must NOT be hand-trimmed; the tests assert against the real fixture (count == 3, at least one shared + one dedicated).
...
Note: the buffer + flush makes the "events() before send()" path correct no matter the actor-scheduling order, and the actor removes the data race the previous `@unchecked Sendable` class had under `liveTicker: true` (the shipping config). The mock never throws `ControlError` — it satisfies `async throws` without failing — but the real transport will, and the ViewModel already handles it (Task 5).
...
- [ ] **Step 7a: Create the deterministic formatter test `TrafficFormattingTests.swift`**

These are the tests that gate the suite — pure functions, host-independent, no
rendering. They run everywhere including headless CI.
...
- [ ] **Step 7b: Create the OPT-IN image snapshot `SnapshotTests.swift`**

The image snapshot is host-dependent (font/antialias/@2x/macOS version) and
needs a window server, so it is OFF by default — `swift test` stays fully
deterministic and headless-safe, which keeps "swift test green" a trustworthy
gate. Run it deliberately with `SNAPSHOT=1 swift test --filter SnapshotTests`
on the dev machine to record/compare a baseline. Do NOT delete baselines to
force a pass.
...
swift test
```
Expected: ALL tests PASS (model/service/view-model/formatter), and `SnapshotTests.testTrafficDashboardSnapshot` reports SKIPPED (it is gated behind `SNAPSHOT=1`). Default `swift test` must be fully green and deterministic — that is the subagent gate. Optionally, on the dev machine, run `SNAPSHOT=1 swift test --filter SnapshotTests` once to record the image baseline (records on first run, compares after); do not force-pass by deleting baselines.

- [ ] **Step 9: Commit**

```bash
cd /Users/liushan/Documents/Personal/Languages/Rust/mini_vpn_app-macos
# __Snapshots__ exists only if you ran SNAPSHOT=1 to record a baseline; the
# `git add` below tolerates its absence.
git add macos-app/Core/Sources/MiniVPNCore/Views \
        macos-app/Core/Tests/MiniVPNCoreTests/TrafficFormattingTests.swift \
        macos-app/Core/Tests/MiniVPNCoreTests/SnapshotTests.swift
git add macos-app/Core/Tests/MiniVPNCoreTests/__Snapshots__ 2>/dev/null || true
git commit -m "feat(macos): SwiftUI screens + deterministic formatter tests + opt-in snapshot"
```
...
Run:
```bash
cd /Users/liushan/Documents/Personal/Languages/Rust/mini_vpn_app-macos/macos-app/App
...
git push origin feat/macos-app
```
...
**Known environment dependencies (not gaps):** `swift test` needs the Swift toolchain (always present on macOS dev). The Task 8 app-shell build needs Xcode + XcodeGen; if absent, it is skipped and noted — the Core package (all logic + tests) is fully validated without it. The image snapshot test is host-rendering-dependent and is therefore OFF by default (gated behind `SNAPSHOT=1`); the default `swift test` suite is deterministic and headless-safe, and the traffic formatters are covered by pure-function tests (`TrafficFormattingTests`). The ① `ControlService` boundary is hardened up front — `send` is `async throws`, the event stream carries `.error`, and `MockControlService` is a lock-protected **multi-subscriber broadcast** (`OSAllocatedUnfairLock`; synchronous attach, `onTermination` detach, fan-out to all live `events()` consumers — no data race under the live ticker, and the app's dual `RootView` / WindowGroup+MenuBarExtra wiring is supported) — so swapping the mock for a real transport is a one-line change with no call-site/UI impact.

---

## Known deferred items (from code-review 2026-06-15)

These were surfaced by the post-stage `/code-review` and consciously deferred —
they are NOT bugs in the current mock-first milestone, but must be addressed when
the real backend ② / local-control ① land. Recorded so they are not mistaken for
"done".

- **Node selection is not yet plumbed to `connect`.** `NodeListViewModel.selectedNodeId`
  is set by the UI / `selectBest()`, but `ConnectionView`'s toggle always issues
  `.auto`; the `selectNode(nodeId:)` / `connect(nodeId:)` command path is never
  driven from the selected node. Today the mock ignores `nodeId` anyway. When the
  real ① core routes by node, wire `selectedNodeId` → `connect(nodeId:)` (and link
  `NodeListViewModel` ↔ `ConnectionViewModel`).
- **Auth token lifecycle has no seam.** `login`/`register`/`refresh` return
  `TokenPair`, but nothing stores or injects it (no token store, no `Authorization`
  header, no refresh-on-401). The "one-line mock→real swap" holds for the data
  shapes but NOT for auth: the real `BackendService` needs a token store threaded
  through (or wrapping) calls, and likely a login gate screen (auth UI is out of
  this milestone by decision). Track as a cross-cutting follow-up.
